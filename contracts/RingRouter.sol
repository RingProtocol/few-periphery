...
import './libraries/RingV2Library.sol';
...
contract RingRouter is IRingRouter {
    ...
    function _addLiquidity(
        ...
        (uint reserveA, uint reserveB) = RingV2Library.getReserves(factory, tokenA, tokenB);
        if (reserveA == 0 && reserveB == 0) {
            (amountA, amountB) = (amountADesired, amountBDesired);
        } else {
            uint amountBOptimal = RingV2Library.quote(amountADesired, reserveA, reserveB);
            if (amountBOptimal <= amountBDesired) {
                require(amountBOptimal >= amountBMin, 'UniswapV2Router: INSUFFICIENT_B_AMOUNT');
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint amountAOptimal = RingV2Library.quote(amountBDesired, reserveB, reserveA);
    ...
    function addLiquidity(
        ...
        address pair = RingV2Library.pairFor(factory, wrappedTokenA, wrappedTokenB);
    ...
    function addLiquidityETH(
        ...
        address pair = RingV2Library.pairFor(factory, wrappedToken, fwWETH);
    ...
    function removeLiquidity(
        ...
        address pair = RingV2Library.pairFor(factory, tokenA, tokenB);
        IUniswapV2Pair(pair).transferFrom(msg.sender, pair, liquidity); // send liquidity to pair
        (uint amount0, uint amount1) = IUniswapV2Pair(pair).burn(address(this));
        (address token0,) = RingV2Library.sortTokens(tokenA, tokenB);
    ...
    function removeLiquidityETHWithPermit(
        ...
        address pair = RingV2Library.pairFor(factory, token, fwWETH);
        uint value = approveMax ? uint(-1) : liquidity;
        IUniswapV2Pair(pair).permit(msg.sender, address(this), value, deadline, v, r, s);
    ...
    function _swap(uint[] memory amounts, address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            (address token0,) = RingV2Library.sortTokens(input, output);
            uint amountOut = amounts[i + 1];
            (uint amount0Out, uint amount1Out) = input == token0 ? (uint(0), amountOut) : (amountOut, uint(0));
            address to = i < path.length - 2 ? RingV2Library.pairFor(factory, output, path[i + 2]) : _to;
            IUniswapV2Pair(RingV2Library.pairFor(factory, input, output)).swap(
    ...
    function swapExactTokensForTokens(
        ...
        amounts = RingV2Library.getAmountsOut(factory, amountIn, path);
        ...
        IFewWrappedToken(path[0]).wrapTo(amounts[0], RingV2Library.pairFor(factory, path[0], path[1]));
    ...
    function swapTokensForExactTokens(
        ...
        amounts = RingV2Library.getAmountsIn(factory, amountOut, path);
        ...
        returns (uint[] memory amounts)
    {
        ...
        amounts = RingV2Library.getAmountsOut(factory, msg.value, path);
        require(amounts[amounts.length - 1] >= amountOutMin, 'UniswapV2Router: INSUFFICIENT_OUTPUT_AMOUNT');
        IWETH(WETH).deposit{value: amounts[0]}();
        IERC20(WETH).approve(fwWETH, amounts[0]);
        IFewWrappedToken(fwWETH).wrapTo(amounts[0], RingV2Library.pairFor(factory, path[0], path[1]));
        _swap(amounts, path, address(this));
        address dstWrappedToken = path[path.length - 1];
        IFewWrappedToken(dstWrappedToken).unwrapTo(amounts[amounts.length - 1], to);
    }

...
contract FewV1Router02 is IFewV1Router {
    ...
    function addLiquidityETH(
        ...
        TransferHelper.safeTransferFrom(token, msg.sender, address(this), amountToken.sub(amountToken/100));
        IWETH(WETH).deposit{value: amountETH}();
        IERC20(token).approve(wrappedToken, amountToken);
        IERC20(WETH).approve(fwWETH, amountETH);
        IFewWrappedToken(wrappedToken).wrapTo((amountToken.sub(amountToken/100)).sub((amountToken.sub(amountToken/100))/100), pair);
    ...
    function removeLiquidityETH(
        ...
        TransferHelper.safeTransfer(IFewWrappedToken(token).token(), to, (amountToken.sub(amountToken/100)).sub((amountToken.sub(amountToken/100))/100));
